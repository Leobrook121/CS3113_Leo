...
void Initialize() {
    ...
    for (int i = 12; i < 19; i++) {
        state.platforms[i].entityType = PLATFORM;
        state.platforms[i].textureID = platformTextureID;
        state.platforms[i].position = glm::vec3(-15.5 + i, 0, 0);
    }
    ...
    state.enemies[0].aiType = DESTROYER;


    state.enemies[1].entityType = ENEMY;
    state.enemies[1].textureID = enemyTextureID;
    state.enemies[1].position = glm::vec3(-0.5, 1, 0);
    ...
    state.enemies[2].position = glm::vec3(0.7, 1, 0);
...
void Render() {
    ...
    if (state.player->lose) {
        DrawText(&program, fontTextureID, "You Lose", 0.7f, -0.25f, glm::vec3(-3.0f, 0, 0));
    }

    bool win = true;
    for (int i = 0; i < ENEMY_COUNT; i++) {
        if (state.enemies[i].isActive) win = false;
    }
    
    if (win) {
        state.player->win = true;
        DrawText(&program, fontTextureID, "You Win", 0.7f, -0.25f, glm::vec3(-3.0f, 0, 0));
    }
